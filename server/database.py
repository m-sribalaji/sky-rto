...
class Narrative(Base):
    """
    Cached plain-English summary of an already-computed Insights/Team
    Rhythm section (e.g. "My WFO Pattern"), generated by an LLM that only
    ever paraphrases numbers this app already computed — it never
    generates a number itself. subject_id is an employee_id for personal
    sections or a team name for team-level ones.

    source_hash is the whole caching strategy: it's a hash of the exact
    facts dict handed to the model for this row. A request only ever
    triggers a new LLM call when that hash has changed since the last
    generation — which happens to solve two problems with one mechanism:
    it naturally caps calls to roughly once per meaningfully-changed day
    (the common case), AND it regenerates immediately the moment a real
    status change (e.g. a WFH->WFO check-in) actually changes the
    underlying numbers, rather than serving a stale narrative until some
    fixed cron next runs.
    """
